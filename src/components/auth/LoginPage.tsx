...
import { Sparkles, Lock, Mail, ArrowRight, AlertCircle, Loader2, Cloud, ShieldCheck } from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isCaptchaVerified) {
      setError('Por favor, marque a caixa de verificação "Não sou um robô" para entrar.');
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden bg-slate-50">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-indigo-100/70 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-100/60 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black tracking-tight text-slate-900 mb-2">
            Entrar no <span className="gradient-text">Núcleo VIP</span>
          </h1>
          <p className="text-sm text-slate-600">
            Acesse o Studio Prompt Pro para engenharia de prompts com IA.
          </p>
        </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-200/90 relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              className="w-full mt-2 py-3 px-4 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white text-sm font-bold shadow-lg shadow-indigo-500/25 flex items-center justify-center gap-2 transition disabled:opacity-50 cursor-pointer"
            >
              {isSubmitting ? (
                <>
          ...
          <div className="mt-6 pt-6 border-t border-slate-100 text-center">
            <p className="text-xs text-slate-500">
              ...
              >
                Comece no Starter VIP por R$ 5,99/mês.
              </button>
            </p>
          </div>
        </div>
